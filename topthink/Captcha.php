...
class Captcha
{
    ...
    protected function writeCurve(): void
    {
        ...
        // 曲线前部分
        $A = mt_rand(1, bcdiv($this->imageH, 2)); // 振幅
        $b = mt_rand(bcdiv(-$this->imageH, 4), bcdiv($this->imageH, 4)); // Y轴方向偏移量
        $f = mt_rand(bcdiv(-$this->imageH, 4), bcdiv($this->imageH, 4)); // X轴方向偏移量
        $T = mt_rand($this->imageH, $this->imageW * 2); // 周期
        $w = (2 * M_PI) / $T;

        $px1 = 0; // 曲线横坐标起始位置
        $px2 = mt_rand(bcdiv($this->imageW, 2), bcmul($this->imageW, 0.8)); // 曲线横坐标结束位置
        ...
        $T   = mt_rand($this->imageH, $this->imageW * 2); // 周期
        $w   = bcdiv((2 * M_PI), $T);
        $b   = (int)($py - $A * sin($w * $px + $f) - $this->imageH / 2);
        $px1 = $px2;
        $px2 = $this->imageW;
